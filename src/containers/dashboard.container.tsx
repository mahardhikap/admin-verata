      ...
      <div className="grid grid-cols-12">
        <div className="col-span-3 flex flex-col text-base font-medium gap-3 bg-main h-screen sticky text-white p-3">
          ...
          {MenuDashboard?.map((item, i) => {
            return (
              <div onClick={()=>router.replace(item.url)} key={i} className={`px-3 py-2 rounded-xl cursor-pointer ${item.url === router.pathname ? 'bg-[#919295]' : ''}`}>
                {item.label}
              </div>
            );
          })}
          <div className="px-3 py-2 border-b-2 border-[#919295] w-fit cursor-pointer" onClick={()=>router.replace('/')}>Logout</div>
        </div>
